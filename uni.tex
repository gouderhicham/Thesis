...
\begin{itemize}
    ...
    \item \textbf{Get contact numbers} to call clinics for an appointment or observation.
\end{itemize}
...
\begin{tikzpicture}[node distance=2cm]
    ...
    \node (found) [decision, below of=viewSchedule] {Found Clinic?};
    \node (call) [process, left of=found, xshift=-4cm] {Call Clinic/Doctor};
    \node (exit) [startstop, below of=found, yshift=-2cm] {Exit};

    \draw [arrow] (start) -- (searchClinic);
    \draw [arrow] (searchClinic.east) -- (searchDoctor.west);
    \draw [arrow] (searchDoctor) -- (viewSchedule);
    \draw [arrow] (viewSchedule) -- (found);
    \draw [arrow] (found.south) -- node[anchor=west] {No} (exit.north);
    \draw [arrow] (found.west) -- node[anchor=south] {Yes} (call.east);
...
\begin{tikzpicture}[node distance=2cm]
    ...
    \node (exit) [startstop, below of=viewSchedule] {Exit};

    \draw [arrow] (start) -- (login);
    \draw [arrow] (login) -- (viewSchedule);
    \draw [arrow] (viewSchedule) -- (exit);
